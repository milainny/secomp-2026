...
function render(day) {
  list.innerHTML = schedule[day].map(([time, title, type, place, host]) => {
    const registration = type === 'Oficina' ? `<button class="register-button" type="button" data-workshop="${title}">Inscrever-se →</button>` : '';
